...
interface QuizState {
  ...
  isTimedOut: boolean
  canEnlarge: boolean
  questionDeadline: Nullable<number>
}
...
const quizStateSlice = createSlice({
  ...
  reducers: {
    ...
    progressQuiz: (state: QuizState, action: PayloadAction<Progress>) => {
      ...
      return {
        ...
        currentSelectorInput: null,
        isTimedOut: isQuestionChange
          ? false
          : (action.payload.isTimedOut ?? state.isTimedOut),
        canEnlarge:
          isQuestionChange || action.payload.isRevealed === true
            ? false
            : state.canEnlarge,
        questionDeadline: isQuestionChange ? null : state.questionDeadline,
      }
    },
    ...
    ensureQuestionDeadline: (
      state: QuizState,
      action: PayloadAction<{ questionId: string; deadline: number }>,
    ) => {
      const currentQuestionId = state.currentQuestions[state.progress]
      if (
  ...
  selectors: {
    ...
    selectCurrentSelectorInput: (state: QuizState) =>
      state.currentSelectorInput,
    ...
    selectQuestionDeadline: (state: QuizState) => state.questionDeadline,
  },
})
